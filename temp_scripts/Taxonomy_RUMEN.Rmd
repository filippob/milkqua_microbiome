...
p <- p + scale_fill_gradient(low = "orange", high = "blue","pvalue")
p <- p + theme(strip.text.y = element_text(size = 7), 
...
q <- q + theme(strip.text = element_text(size = 6), 
               strip.text.x = element_text(size = 8),
...
figure_final <- ggarrange(q, p, widths=c(0.3, 1), labels=c("A","B"), label.x = -0.015, common.legend = TRUE, legend = "right")

# print(figure_final)
ggsave(filename = "~/Results/RUMEN/results/heatmap_rumen.png", plot = figure_final, device = "png", width = 10, height = 3)
...
<!-- dd <- spread(D0, key = treatment, value = avg_counts) -->
<!-- temp <- inner_join(DX,dd, by = c("level" = "level", "new_taxa" = "new_taxa")) -->
<!-- fwrite(temp, file = file.path("/home/mycelium/Results/RUMEN/rumen_significant_otus_pre.csv"), col.names = TRUE, sep = ",") -->
<!-- print (dd) -->
<!-- ``` -->
