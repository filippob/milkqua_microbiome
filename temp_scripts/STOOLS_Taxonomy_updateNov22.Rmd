...
#  filter(avg_abund>0.05) %>% #ONE FILTER HERE
...
D$timepoint <- factor(D$timepoint, levels=c("T2")) #CHANGE HERE
...
Dna$timepoint = factor(Dna$timepoint, levels = c("T2"))
...
fg <- fg + coord_flip() + scale_fill_manual(values = c("#0072B2", "orchid", "blue")) #2
...
print(FF)
ggsave(filename = "~/Results/STOOLS/results/T2/taxonomy_graph_T2.png", plot = FF, device = "png", width = 12, height = 6)
```
