...
Dna$timepoint = factor(Dna$timepoint, levels = c("day 0","day 7","day 21","day 28"))
...
ggsave(filename = "~/Results/Milkqua_milk_invivopublication/results/taxonomy/barplot_pval-phyla.png", plot = FF, device = "png", width = 15, height = 1.2)
```
...
uncult <- slice(otu_genus, 1, 370:379)
uncult$Genus <- "Uncultured or unknown"
uncult <- uncult %>%
  group_by(Genus) %>%
  summarise(across(everything(), sum))

otu_genus <- otu_genus[-c(1, 370:379), ]
...
FF <- ggarrange(q, NULL, fg, widths=c(0.5,0.025,1), heights=c(1,1,1), labels = c("A","","B"), label.x = -0.015 , nrow = 1)
print(FF)
