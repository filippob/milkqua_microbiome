...
```{r data, echo=FALSE}
metadata <- fread("/home/mycelium/Results/mapping_file_milk_alltimepoints_skin_T0beforeafter.csv")
...
otu <- read.table("/home/mycelium/Results/results/taxa_summary_abs/mapping_file_L2.txt",header=TRUE, sep="\t" ,comment.char = "&")
names(otu)[1] <- "sample"
...
D <- m1 %>%
  ...
  filter(avg_abund>0.05) %>% ### AVG ABUND IS 0.005 OR 0.05? ORIGINALLY HERE WAS WRITTEN 0.005. IMAGE DOES NOT CHANGE THAT MUCH
...
otu <- fread("/home/mycelium/Results/results/taxa_summary_abs/mapping_file_L6.txt",header=TRUE)
names(otu)[1] <- "sample"
...
p
#ggsave (p, height = 13, width = 8, filename = "taxonomy_bubbleplot_MILK.png")
```
