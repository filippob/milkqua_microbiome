...
otu_phylum <- cbind(otu_phylum, treatment=metadata$treatment, `timepoint-symb`=metadata$`timepoint-symb`, sample=metadata$sample)
...
Pphyla <- Pphyla + facet_grid(~treatment)
...
my_palette = get_palette(c("green3","gold","blue","magenta","cyan","yellow","red","gray","purple","brown", "darkorange1","pink1", "darkorchid1", "seagreen2"), length(unique(D$phylum)))
...
print(Pphyla)
ggsave(Pphyla, height = 5, width = 5, filename ="~/Results/SKINSWABS/results/taxonomy_phylum_boxplot_skinswab_tot.png")
...
```{r, echo=FALSE}
otu_genus <- cbind(otu_genus, treatment=metadata$treatment, `timepoint-symb`=metadata$`timepoint-symb`, sample=metadata$sample)
...
print(P)
ggsave(P, height = 12, width = 20, filename ="~/Results/SKINSWABS/results/taxonomy_genus_boxplot_skinswab_tot.png")
...
otu_genus = subset(otu_genus, select = -c(`timepoint-symb`, treatment, sample))
otu_phylum = subset(otu_phylum, select = -c(`timepoint-symb`, treatment, sample))
...
otu_tot$`timepoint-symb`= metadata$`timepoint-symb`[match(otu_tot$sample,metadata$sample)]
...
p
ggsave (p, height = 13, width = 8, filename = "~/Results/SKINSWABS/results/taxonomy_bubbleplot_rumen.png")
```
...
q <- q + xlab("") 
q
#ggsave(filename = "~/Results/SKINSWABS/results/heatmap_pvalue_rumen.png", plot = q, device = "png", width = 6, height = 6)


figure_final <- ggarrange(q, p, widths=c(0.1, 0.1), labels=c("A", "B"), legend = "bottom")

print(figure_final)
ggsave(filename = "~/Results/SKINSWABS/results/heatmap_rumen.png", plot = figure_final, device = "png", width = 15, height = 12)
```
...
write.csv(res, "~/Results/SKINSWABS/results/res.csv", sep = ",")
```