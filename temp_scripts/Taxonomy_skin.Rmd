...
D$timepoint <- factor(D$timepoint, levels=c("before_oil", "after_oil", "8"))
...
D_genus$timepoint <- factor(D_genus$timepoint, levels=c("before_oil", "after_oil", "8"))
...
figure_final <- ggarrange(q, p, widths=c(0.04, 0.1), labels=c("A", "B"), legend = "bottom")

print(figure_final)
ggsave(filename = "~/Results/SKINSWABS/results/taxonomy_heatmap_skinswabs.png", plot = figure_final, device = "png", width = 15, height = 12)
```
...
fg <- fg + coord_flip()
fg
ggsave(filename = "~/Results/SKINSWABS/results/taxonomy_barplot_significantOTUS_genera_skinswabs.png", plot = fg, device = "png", width = 12, height = 12)
```
