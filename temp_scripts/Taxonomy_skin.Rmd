...
#ggsave(filename = file.path(basedir, prjdir, outdir, "taxonomy_Phylum_piechart_skinswabs_wbaseline.png"), plot = p, device = "png", width = 14, height = 7)
```

#### Core microbiota: family, genus etc. continue if needed = replace Phylum with Genus or whatevere you are interested in. Careful: blank spaces can suggest that it was not possible to identify that taxa level. --> solve this problem?
...
```{r}
temp = select(otu, contains("sample"), Phylum #, Genus, Kingdom, Species, Class, Order, Family
              )%>% gather(key = "sample", value = "counts", -Phylum #, -Genus, -Kingdom, -Species, -Class, -Order, -Family
                          ) %>%  group_by(Phylum #, Genus, Kingdom, Species, Class, Order, Family
                                          , sample) %>%
...
```{r}
temp <- temp %>% 
  inner_join(bl, by = c("Phylum" = "Phylum", #"Genus"="Genus", "Kingdom"="Kingdom", "Species"="Species", "Class"="Class", "Order"="Order", "Family"="Family", 
                        "treatment" = "treatment")) %>%
...
p <- ggplot(data=dd, aes(x=timepoint, y=avg, group=treatment)) +
  ...
  geom_point()
p <- p + facet_wrap(~Phylum, scales = "free") #change desidered taxa level after in "facet_wrap(~" 
p
```
...
D <- temp %>%
  group_by(Phylum) %>%
  summarise(N=n(), avg_abund = round(mean(tot),4)*0.01, std = round(sd(tot),3)) %>%
...
```{r, echo=FALSE}
dd <- temp %>%
  group_by(Phylum, Genus, Kingdom, Species, Class, Order, Family,timepoint,treatment) %>%
  summarise(avg = mean(tot)) %>%
  spread(key = treatment, value = avg)

d0 <- select(D, c(Phylum, Genus, Kingdom, Species, Class, Order, Family, p.value))

dd <- dd %>% inner_join(d0, by = c("Phylum" = "Phylum", "Genus"="Genus", "Kingdom"="Kingdom", "Species"="Species", "Class"="Class", "Order"="Order", "Family"="Family"))
...
Dna$CONTROL<-NULL
Dna$TREATED<-NULL
Dna <- Dna %>% gather(Genus, p.value)

DnaT1 <- subset(Dna, timepoint == "T1")
...
fg <- fg + coord_flip()
fg <- fg + ylab("Behaviour of the Treated against Control baseline, in significant p_values") + xlab("Taxa")
