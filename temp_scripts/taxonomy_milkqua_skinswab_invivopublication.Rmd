...
# write.csv(Dna, "~/Results/STOOLS/results/T2/results_T2.csv")

Dna$timepoint = factor(Dna$timepoint, levels = c("day 0A","day 0B","day 7"))
...
fg <- fg + ylab("Behaviour Treated vs. Control") + xlab("Taxa") + ylim(-0.03, 0.03)
...
ggsave(filename = "~/Results/Milkqua_skinswab_invivopublication/results/taxonomy/barplot_pval-phyla_SKINSWAB.png", plot = FF, device = "png", width = 15, height = 2)
```

#core microbiota genus level

```{r}
otu_genus=select(otu, 2:48,54)
otu_genus <- otu_genus %>% group_by(Genus) %>% summarise(across(everything(),sum))

uncult <- slice(otu_genus, 1, 561:578)
uncult$Genus <- "Uncultured or unknown"
uncult <- uncult %>%
  group_by(Genus) %>%
  summarise(across(everything(), sum))

otu_genus <- otu_genus[-c(1, 561:578), ]
...
pgenera <- pgenera + scale_fill_manual(values = c("red", "green", "khaki", "skyblue", "lightgrey", "blue1", "red2", "lightyellow", "steelblue1","maroon","lightblue","salmon","turquoise","violet","yellow","gray","tomato","navy","pink","springgreen4","peru","sienna1","plum4","mediumblue","darkorange","brown4","gold","bisque4"))
...
Dna$timepoint = factor(Dna$timepoint, levels = c("day 0A","day 0B","day 8"))
...
# + scale_fill_manual(values = "#E69F00", #0072B2",) #3
fg <- fg + coord_flip() + scale_fill_manual(values = c("#FFC20A", "#0C7BDC", "#e66100", "#40b0A6" )) #colorblind safe 
fg <- fg + ylab("Behaviour Treated vs. Control") + xlab("Taxa") + ylim(-0.003, 0.003)
fg <- fg + theme(axis.text.x = element_text(size = 11),
                 axis.text.y = element_blank(),
                 axis.ticks = element_blank(),
                 # legend.position = "bottom",
