---
title: "Milkqua_skinswab_invivopublication"
author: "Chiara Gini"
date: "2023-01-18"
...
```{r , echo=FALSE}
basedir="~/Results"
prjdir = "Milkqua_skinswab_invivopublication"
outdir = "results"
...
metadata <- subset(metadata, timepoint != "T0b")

meta <- metadata [,c(1,12,13)]
meta_cols = names(meta)
metadata <- metadata[!(metadata$sample=="sample-29"),] #discarded due to low quality

otu <- fread("/home/mycelium/Results/Milkqua_skinswab_invivopublication/results/otu_norm_CSS.csv", header = TRUE, sep = ",")
otu <- filter(otu, Family !="Mitochondria")
otu <- filter(otu, Class !="Chloroplast")
otu <- filter(otu, Order !="Chloroplast")

vec1 <- metadata$sample
N <- otu %>% select(all_of(vec1))
N$Phylum <- otu$Phylum
N$Genus <- otu$Genus
```

```{r}
IF YOU NEED TO CHECK NUMBER OF otuS
...
pphyla <- pphyla + theme(text = element_text(size=20),
...
my_palette2 = get_palette(c("chartreuse4", "magenta","cyan","yellow","red","coral", "gray", "mediumorchid3", "turquoise4","yellow3",  "green3","gold","blue","purple", "darkorange1","pink1","seagreen2", "darkmagenta", "burlywood3", "darkorchid1",  "slategray3", "orangered", "olivedrab", "plum", "lightcyan", "gray38", "forestgreen", "firebrick1"), length(unique(D$phylum)))
...
boxplotP <- p
print(boxplotP)
ggsave(p, height = 5, width = 9, filename ="~/Results/Milkqua_skinswab_invivopublication/results/taxonomy/phylum_boxplot_SKINSWAB.png")
```
...
save(m1, file = "~/Results/Milkqua_skinswab_invivopublication/results/taxonomy/Phylum_signif_SKINSWAB.RData")
...
save(to_save, file = "~/Results/Milkqua_skinswab_invivopublication/results/taxonomy/taxonomy-phyl_SKINSWAB.RData")

load("~/Results/Milkqua_skinswab_invivopublication/results/taxonomy/taxonomy-phyl_SKINSWAB.RData")
D <- to_save[[1]]
DX <- to_save[[2]]
D0 <- to_save[[3]]
...
fwrite(temp, file = "~/Results/Milkqua_skinswab_invivopublication/results/taxonomy/significant_otus-phyl_SKINSWAB.csv", col.names = TRUE, sep = ",")
print (dd)
```
...
write.csv(res, "~/Results/Milkqua_skinswab_invivopublication/results/taxonomy/pvalues-phyl_SKINSWAB.csv")
```
...
## write out table of the core microbiota
ffname = file.path(basedir, prjdir, outdir, "taxonomy/core_microbiota_SKINSWAB.csv")
select(A, c(taxon,avg)) %>% rename(avg_normalised_counts = avg) %>% fwrite(ffname, sep = ",")
...
pgenera <- pgenera + scale_fill_manual(values = c("red", "green", "khaki", "skyblue", "lightgrey", "blue1", "red2", "lightyellow", "steelblue1","maroon","lightblue","salmon","turquoise","violet","yellow","gray","tomato","navy","pink","springgreen4","peru","sienna1","plum4","mediumblue","darkorange","brown4","gold","bisque4", "lawngreen", "pink4", "khaki", "turquoise", "tan", "greenyellow", "goldenrod1"))
pgenera <- pgenera + theme(text = element_text(size=20),
               ...
               legend.text=element_text(size=20),
               legend.title=element_text(size=20))
pgenera <- pgenera + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), legend.text = element_blank()) 
pgenera

g <- ggarrange(pphyla, pgenera, ncol = 2, labels = c("A","B"), heights= c(1,1), widths = c(0.5,1)) #heights = c(0.1,4))
g

# ggsave(filename = "~/Results/Milkqua_skinswab_invivopublication/results/taxonomy/piechart_core_microbiome_SKINSWAB.png", plot = p, device = "png", dpi = 250, width = 21, height = 13)
...
boxplotG <- p
print(boxplotG)
ggsave(p, height = 5, width = 12, filename ="~/Results/Milkqua_skinswab_invivopublication/results/taxonomy/genera_boxplot_SKINSWAB.png")
```
...
m1 <- mO %>%
  arrange(level,treatment)
save(m1, file = "~/Results/Milkqua_skinswab_invivopublication/results/taxonomy/Genus_signif_SKINSWAB.RData")
...
to_save = list(D,DX,D0)
save(to_save, file = "~/Results/Milkqua_skinswab_invivopublication/results/taxonomy/taxonomy-gen_SKINSWAB.RData")

load("~/Results/Milkqua_skinswab_invivopublication/results/taxonomy/taxonomy-gen_SKINSWAB.RData")
D <- to_save[[1]]
DX <- to_save[[2]]
D0 <- to_save[[3]]
...
dd <- spread(D0, key = treatment, value = avg_counts)
temp <- inner_join(DX,dd, by = c("level" = "level", "taxa" = "taxa", "timepoint"="timepoint"))
fwrite(temp, file = "~/Results/Milkqua_skinswab_invivopublication/results/taxonomy/significant_otus-gen_SKINSWAB.csv", col.names = TRUE, sep = ",")
print (dd)
```
...
write.csv(res, "~/Results/Milkqua_skinswab_invivopublication/results/taxonomy/pvalues-gen_SKINSWAB.csv")
```
...
Dna$timepoint = factor(Dna$timepoint, levels = c("T0","T7"))

fwrite(Dna, file = "~/Results/Milkqua_skinswab_invivopublication/results/taxonomy/useful_SKINSWAB.csv", col.names = TRUE, sep = ",")
...
fg <- fg + ylab("Behaviour Treated vs. Control") + xlab("Taxa") + ylim(-0.008, 0.0025)
fg <- fg + theme (axis.text.x = element_text(size = 13), axis.text.y = element_blank(), axis.ticks = element_blank(), axis.title.x = element_text(size=13), axis.title.y =  element_blank())
fg

write.csv(Dna, "~/Results/Milkqua_skinswab_invivopublication/results/taxonomy/skin-genera-behav.csv")

FF <- ggarrange(q, fg, widths=c(0.35,0.65), heights=c(1,1))
print(FF)

ggsave(filename = "~/Results/Milkqua_skinswab_invivopublication/results/taxonomy/tot-genera-skin.png", plot = FF, device = "png", dpi = 300, width = 20, height = 6.5)

#FF <- ggarrange(q, fg, widths=c(0.5,1), heights=c(1,1), labels = c("A","B"))
#print(FF)
#ggsave(filename = "~/Results/Milkqua_skinswab_invivopublication/results/taxonomy/taxonomy_barplot_pval-gen_SKINSWAB.png", plot = FF, device = "png", width = 15, height = 9)
```
...
load("~/Results/Milkqua_skinswab_invivopublication/m1.Rdata")
m1 = to_save[[1]]
unique(m1$treatment)
