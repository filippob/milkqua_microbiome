...
mAlpha$timepoint <- factor(mAlpha$timepoint, levels = c("day 0A","day 0B","day 8"))
D <- mAlpha %>%
  group_by(metric,treatment, timepoint) %>%
  summarize(N=n(),avg=round(mean(value),3)) %>%
  spread(key = metric, value = avg)

D$timepoint <- factor(D$timepoint, levels = c("day 0A","day 0B","day 8"))
...
mD$timepoint <- factor(mD$timepoint, levels= c("day 0A","day 0B","day 8"))
...
ggsave(filename = "~/Results/Milkqua_skinswab_invivopublication/results/alpha/alpha_SKINSWAB.png", plot = figure_finalA, device = "png", width = 15, height = 5)
```