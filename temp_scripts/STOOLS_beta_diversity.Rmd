...
Clustering by treatment T2
(PCA)
```{r, echo=FALSE}
p2 <- autoplot(prcomp(1-matx), data=matrice, colour = "treatment", label = TRUE, label.size = 3, frame = TRUE, frame.type='norm') + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
p2
...
```{r, echo=FALSE}
p3 <- autoplot(prcomp(1-matx), data=matrice, colour = "treatment", label = TRUE, label.size = 3, frame = TRUE, frame.type='norm') + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
p3
# ggsave("~/Results/STOOLS/results/clustering_by_treatment_PCA_stools.png")
```

```{r}
figure_finalB <- ggarrange(p2, p3, widths=c(1, 1), labels=c("T2", "T3"), legend = "bottom")
print(figure_finalB)

#ggsave("~/Results/STOOLS/results/clustering_merged_PCA_skinswab300822.png", plot = figure_final, width = 14, height = 5)
```
