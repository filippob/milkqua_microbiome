...
metadata <- fread("/home/mycelium/Results/Milkqua_skinswab_bootstrapping/mapping_milkqua_skinswabs.csv")
names(metadata)[1] <- "sample"
names(metadata)[10] <- "type"

metadata$timepoint[metadata$timepoint == "before_oil"] <- "T1"
metadata$timepoint[metadata$timepoint == "after_oil"] <- "T2"
metadata$timepoint[metadata$timepoint == "8"] <- "T3"
metadata <- metadata [order(metadata$sample),]

meta <- metadata [,c(1,13)]
meta_cols = names(meta)
metadata <- metadata[!(metadata$treatment=="Treated"),] 
...
p <- p + guides(fill = guide_legend(title = "Phyla")) + coord_polar(theta='y')
...
print(p)
ggsave(p, height = 5, width = 9, filename ="~/Results/Milkqua_skinswab_bootstrapping/results/taxonomy/phylum_boxplot_SKINSWAB.png")
```
...
m1 <- mO %>%
  filter(!is.na(level)) %>%
  arrange(level)
save(m1, file = "~/Results/Milkqua_skinswab_bootstrapping/results/taxonomy/Phylum_signif_SKINSWAB.RData")
...
write.csv(res, "~/Results/Milkqua_skinswab_bootstrapping/results/taxonomy/pvalues-phyl_SKINSWAB.csv")
```
...
ggsave(filename = "~/Results/Milkqua_skinswab_bootstrapping/results/taxonomy/barplot_pval-phyla_SKINSWAB.png", plot = FF, device = "png", width = 15, height = 2)
```
...
# ggsave(filename = "~/Results/Milkqua_skinswab_bootstrapping/results/taxonomy/piechart_core_microbiome_SKINSWAB.png", plot = p, device = "png", dpi = 250, width = 21, height = 13)
...
p <- p + theme(text = element_text(size=10),
               ...
               legend.title=element_text(size=8))
print(p)
ggsave(p, height = 5, width = 12, filename ="~/Results/Milkqua_skinswab_bootstrapping/results/taxonomy/genera_boxplot_SKINSWAB.png")
```
...
m1 <- mO %>%
  arrange(level,treatment)
save(m1, file = "~/Results/Milkqua_skinswab_bootstrapping/results/taxonomy/Genus_signif_SKINSWAB.RData")
...
to_save = list(D,DX,D0)
save(to_save, file = "~/Results/Milkqua_skinswab_bootstrapping/results/taxonomy/taxonomy-gen_SKINSWAB.RData")

load("~/Results/Milkqua_skinswab_bootstrapping/results/taxonomy/taxonomy-gen_SKINSWAB.RData")
D <- to_save[[1]]
DX <- to_save[[2]]
D0 <- to_save[[3]]
...
temp <- inner_join(DX,dd, by = c("level" = "level", "taxa" = "taxa", "timepoint"="timepoint"))
fwrite(temp, file = "~/Results/Milkqua_skinswab_bootstrapping/results/taxonomy/significant_otus-gen_SKINSWAB.csv", col.names = TRUE, sep = ",")
print (dd)
```
...
write.csv(res, "~/Results/Milkqua_skinswab_bootstrapping/results/taxonomy/pvalues-gen_SKINSWAB.csv")
```
...
fwrite(Dna, file = "~/Results/Milkqua_skinswab_bootstrapping/results/taxonomy/useful_SKINSWAB.csv", col.names = TRUE, sep = ",")
...
ggsave(filename = "~/Results/Milkqua_skinswab_bootstrapping/results/taxonomy/taxonomy_barplot_pval-gen_SKINSWAB.png", plot = FF, device = "png", width = 15, height = 9)
```
...
load("~/Results/Milkqua_skinswab_bootstrapping/m1.Rdata")
m1 = to_save[[1]]
unique(m1$treatment)
