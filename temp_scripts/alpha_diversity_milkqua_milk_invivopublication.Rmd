...
mAlpha$timepoint <- factor(mAlpha$timepoint, levels = c("day 0","day 7","day 21","day 28"))
D <- mAlpha %>%
  group_by(metric,treatment, timepoint) %>%
  summarize(N=n(),avg=round(mean(value),3)) %>%
  spread(key = metric, value = avg)

D$timepoint <- factor(D$timepoint, levels = c("day 0","day 7","day 21","day 28"))
...
ggsave(filename = "~/Results/Milkqua_milk_invivopublication/results/alpha/alpha_MILK.png", plot = figure_finalA, device = "png", width = 15, height = 5)
```