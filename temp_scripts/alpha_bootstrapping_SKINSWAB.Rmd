...
```{r}
indices <- colnames(alpha)[10]

M <- M[-c(3,6,8)]

temp <- M %>% spread(metric, corrected_counts)
```

```{r, echo=FALSE}
res = data.frame("index"=NULL, "stat"=NULL, "pval"=NULL, "coef"=NULL, "timepoint"=NULL)

M0 = filter(M, timepoint != "T0")
for (k in indices) {
  ...
  mm = M0[M0$metric == k,]
   ...
   tbl = temp %>%
    ...
    t1 = pull(tbl,T1)
    ctrl_t1 <- sum(sum(t1>0) == 2, min(t1) > 3) == 2
    t2 = pull(tbl,T2)
    ctrl_t2 <-  sum(sum(t2>0) == 2, min(t2) > 3) == 2
