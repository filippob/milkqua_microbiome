...
alpha <- read.table("/home/oem/milkqua_microbiome/RUMEN/results/alpha_diversity/alpha.txt", header = TRUE)
...
mAlpha$treatment <- factor(mAlpha$treatment, levels = c("no treatment (ruminal liquid + diet)","AE1","AE sintético 1","Carvacrol", "p-cymene", "γ-terpinene", "ruminal liquid"))
D <- mAlpha %>%
...
```{r}
bl_medie <- mAlpha %>%
  filter(treatment=="no treatment (ruminal liquid + diet)") %>%
  group_by(metric) %>%
...
p <- ggplot(mD, aes(x=treatment,y=value, group=treatment)) + geom_point(aes(colour=treatment))
