...
joined_pairs <- read.csv("results/readsPerSample",sep="\t",header = FALSE,skip = 1, stringsAsFactors = FALSE)
...
inputSeqs <- read.table("results/inputSeqs",sep=":")
outputSeqs <- read.table("results/outputSeqs", sep="\t")
samples <- read.table("results/samples.txt", sep=":")
...
```{r, echo=FALSE}
mapfile <- fread("mapping_file.csv", header = TRUE)
