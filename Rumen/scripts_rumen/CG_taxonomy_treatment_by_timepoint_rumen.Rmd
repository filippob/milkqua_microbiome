...
otu <- read.table("/home/mycelium/milkqua_project/Rumen/results/taxa_summary_abs/CSS_normalized_otu_table_L2_MOD.csv",header=TRUE, sep=",", comment.char = "&")

otu$treatment = metadata$treatment[match(otu$ID,metadata$Concatenated)]
otu$timepoint = metadata$timepoint[match(otu$ID,metadata$Concatenated)]
otu <- otu %>% relocate (timepoint, .after = ID)
otu <- otu %>% relocate (treatment, .after = timepoint)

names(otu)[1] <- "sample"

## relative abundances
metadata <- metadata %>% relocate (Concatenated, .after = sample)
metadata <- metadata %>% relocate (timepoint, .after = treatment)

## relative abundances
metadata_cols = seq(2,4)
M <- otu[,-metadata_cols]
M <- M/rowSums(M)
...
D <- m1 %>%
  ...
  summarise(N=n(), avg_abund = round(mean(counts),2), std = round(sd(counts),3)) %>%
...
p <- p + scale_fill_gradient2(low="red",high = "darkgreen")
p
...
p <- p + scale_fill_brewer(palette = "Set3") + geom_label_repel(aes(label = paste0(avg_abund*100, "%")), position=position_stack (vjust=0.5), size=4, show.legend = F) + guides(fill = guide_legend(title = "Phyla")) 
...
print(p)
ggsave(p, height = 20, width = 10, filename ="taxonomy_phylum_piechart_rumen.png")
...
<!-- print(p) -->
<!-- ggsave(p, height = 20, width = 4, filename ="taxonomy_phylum_boxplot_rumen.png") -->
...
p <- p + facet_grid(timepoint~treatment)
p <- p + scale_fill_brewer(palette = "Set3") +  guides(fill = guide_legend(title = "Phyla")) 
p <- p + xlab("relative abundances") + ylab("percentages")
# p <- p + scale_fill_manual(values = my_palette)
print(p)
ggsave(p, height = 16, width = 4, filename ="taxonomy_phylum_barplot_rumen.png")
...
my_palette2 = get_palette(c("#00AFBB", "#E7B800", "#FC4E07","darkgrey","beige"), length(unique(D$new_taxa)))
...
p <- p + theme(text = element_text(size=10),
               ...
               legend.title=element_text(size=10))
print(p)
ggsave(p, height = 8, width = 8, filename ="taxonomy_phylum_bargraph_rumen.png")
...
ggsave ("taxonomy_bubbleplot_rumen.png")
```
