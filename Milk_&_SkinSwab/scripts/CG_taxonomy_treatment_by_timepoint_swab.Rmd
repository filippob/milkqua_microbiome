---
title: "taxonomy per treatment and timepoint: skin swab samples"
author: "Chiara Gini"
...
otu <- read.table("/home/mycelium/milkqua_project/qiime_1.9_results/results/taxa_summary_abs/mapping_file_L2.txt",header=TRUE, sep="\t" ,comment.char = "&")
names(otu)[1] <- "sample"
...
p <- p + ylab("sample")
p <- p + theme(axis.text.x = element_text(angle = 90, hjust = 1),axis.text.y = element_text(size=18))
p <- p + scale_fill_gradient2(low="ivory",high = "red")
p

ggsave ("taxonomy_count_swab.png", width = 15, height= 10)
```
...
print(p)
ggsave(p, height = 20, width = 10, filename ="taxonomy_phylum_piechart_swab.png")
```
...
# p <- p + scale_fill_manual(values = my_palette)
print(p)
ggsave(p, height = 8, width = 4, filename ="taxonomy_phylum_boxplot_swab.png")
...
formattable(D0, list(
  ...
  `before_oil` = color_tile("white","yellow"),
  `after_oil` = color_tile("white","pink")))
...
ggsave ("taxonomy_bubbleplot_swab.png")
```
...
ggsave(filename = "heatmap_swab.pdf", plot = figure_final, device = "pdf", width = 8, height = 12)
```
