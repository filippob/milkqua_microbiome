...
matrice= read.table("/home/mycelium/milkqua_project/qiime_1.9_results/results/beta_diversity/bray_curtis_CSS_normalized_otu_table.txt", row.names=1, header=T)
...
Before_oil
```{r, echo=FALSE}
mat_swab_1 <- filter(mat_swab, timepoint =="before_oil" )
matxTP= data.matrix(select(mat_swab_1, -c(treatment,timepoint)))

p2 <- autoplot(prcomp(1-matxTP), data=mat_swab_1, colour = "treatment", label = TRUE, label.size = 3, frame = TRUE, frame.type='norm')
p2
ggsave("clustering_by_treatment_PCA_swab_before_oil.png")
```

After_oil
```{r, echo=FALSE}
mat_swab_1 <- filter(mat_swab, timepoint =="after_oil" )
matxTP= data.matrix(select(mat_swab_1, -c(treatment,timepoint)))

p2 <- autoplot(prcomp(1-matxTP), data=mat_swab_1, colour = "treatment", label = TRUE, label.size = 3, frame = TRUE, frame.type='norm')
p2
ggsave("clustering_by_treatment_PCA_swab_after_oil.png")
```
...
udder.mds= metaMDS(matxTP, k=3) #function metaMDS in Vegan
...
row.names(obj$aov.tab) <- c("treatment","timepoint","treatment_by_timepoint","residuals") # IT WAS ("treatment","timepoint","treatment_by_timepoint","residuals","total")
kable(obj$aov.tab)
```
...
if(pandoc_available()) {
  ...
  htmlwidgets::saveWidget(as_widget(p), "beta_timepoint_3d_swab.html")
}
```
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
temp <- filter(udder.scores, timepoint == "before_oil")
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
temp <- filter(udder.scores, timepoint == "after_oil")
